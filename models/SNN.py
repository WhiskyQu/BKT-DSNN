...
class SNN_VGG9_BNTT(nn.Module):
    def __init__(self, in_channels, timesteps, leak_mem, img_size, num_cls):
        ...
        #     self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        # else:
        #     self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.conv1 = nn.Conv2d(self.in_channels, 64, kernel_size=3, stride=1, padding=1, bias=bias_flag)
...
if __name__ == '__main__':
    ...
    model = SNN_VGG11_BNTT(timesteps=timesteps,
                          leak_mem=leak_mem,
                          img_size=img_size,
                          in_channels=3,
                          num_cls=num_cls)
