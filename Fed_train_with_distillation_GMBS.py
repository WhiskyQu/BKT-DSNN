...
def GMBS(client_data_distribution, client_compute_power, client_communication_bandwidth, total_blocks):
    ...
    norm_data_distribution = client_data_distribution
    norm_compute_power = client_compute_power
    norm_communication_bandwidth = client_communication_bandwidth

    score = alpha * norm_data_distribution + beta * norm_compute_power + gamma * norm_communication_bandwidth

    block_allocation_ratio = score  

    return block_allocation_ratio

def federated_learning_with_gmbs(model_class, trainloader, testloader, criterion, device, rounds, local_epochs, client_selection_ratio, num_clients, model_args, save_path, save_frequency, total_blocks):
    ...
    for round in range(rounds):
        ...
        for client_idx in selected_clients:
            ...
            block_allocation_ratio = GMBS(data_distribution, compute_power, communication_bandwidth, total_blocks)
            # block_allocation_ratio = GMBS(client_data_distribution[client_idx], client_compute_power[client_idx], client_communication_bandwidth[client_idx], total_blocks)
            block_allocation = int(block_allocation_ratio * total_blocks)  
            print(f"Client {client_idx} assigned {block_allocation} blocks based on its capability")

            prune_ratio = block_allocation / total_blocks 
            # client_model = model_class(*model_args, prune_ratio=prune_ratio).to(device)
            client_model = model_class(*model_args).to(device)
            print(f"Training client {client_idx} for {local_epochs} local epochs with prune ratio {prune_ratio:.2f}")
            train_model(client_model, trainloader, criterion, client_optimizers[client_idx], device, local_epochs)
...
def train_model_with_distillation(student_model, teacher_model, dataloader, criterion, optimizer, device, epochs):
    student_model.train()
    teacher_model.eval()
    ...
    for epoch in range(epochs):
        ...
        for images, labels in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{epochs}", unit="batch"):
            ...
            total_loss = alpha * distillation_loss + (1 - alpha) * loss
...
def main():
    ...
    dataset_path_mapping = {
        ...
        'mnist': (
        '../data/MNIST/npy/train_images.npy', '../data/MNIST/npy/train_labels.npy', '../data/MNIST/npy/test_images.npy',
    ...
    federated_learning_with_gmbs(model_mapping[args.model_name], trainloader, testloader, criterion, device, args.rounds, args.local_epochs, args.client_selection_ratio, args.clients, model_args, args.model_save_path, args.save_frequency, 18)
