...
def GMBS(client_data_distribution, client_compute_power, client_communication_bandwidth, total_blocks, history=None, alpha=0.5, beta=0.3, gamma=0.2, smoothing=0.1):
    if isinstance(client_data_distribution, (float, int)):
        client_data_distribution = [client_data_distribution]
    if isinstance(client_compute_power, (float, int)):
        client_compute_power = [client_compute_power]
    if isinstance(client_communication_bandwidth, (float, int)):
        client_communication_bandwidth = [client_communication_bandwidth]

    norm_data_distribution = np.array(client_data_distribution) / np.sum(client_data_distribution)
    norm_compute_power = np.array(client_compute_power) / np.sum(client_compute_power)
    norm_communication_bandwidth = np.array(client_communication_bandwidth) / np.sum(client_communication_bandwidth)

    alpha = smoothing * alpha + (1 - smoothing) * (0.5 * (1 + np.std(client_data_distribution)))
    beta = smoothing * beta + (1 - smoothing) * (0.3 * (1 + np.std(client_compute_power)))
    gamma = smoothing * gamma + (1 - smoothing) * (0.2 * (1 + np.std(client_communication_bandwidth)))

    current_score = alpha * norm_data_distribution + beta * norm_compute_power + gamma * norm_communication_bandwidth

    if history is not None:
        current_score = smoothing * history + (1 - smoothing) * current_score

    block_allocation_ratio = current_score / np.sum(current_score)

    return list(block_allocation_ratio), current_score
...
def federated_learning_with_gmbs_ktl(model_class, trainloader, testloader, criterion, device, rounds, local_epochs, client_selection_ratio, num_clients, model_args, save_path, save_frequency, total_blocks):
    ...
    for round in range(rounds):
        ...
        block_allocation_ratio, history_score = GMBS(
            [client_data_distribution[i] for i in selected_clients],
            [client_compute_power[i] for i in selected_clients],
            [client_communication_bandwidth[i] for i in selected_clients],
            total_blocks,
            history=history_score
        )

        block_allocation = [int(ratio * total_blocks) for ratio in block_allocation_ratio]

        print(f"Block allocation: {block_allocation}")

        for client_idx in selected_clients:
            data_distribution = [client_data_distribution[client_idx]]
            compute_power = [client_compute_power[client_idx]]
            communication_bandwidth = [client_communication_bandwidth[client_idx]]

            block_allocation_ratio, _ = GMBS(data_distribution, compute_power, communication_bandwidth, total_blocks)
            block_allocation = int(block_allocation_ratio[0] * total_blocks)  # 这里取第一个元素
            print(f"Client {client_idx} assigned {block_allocation} blocks based on its capability")

            print(f"Training client {client_idx} for {local_epochs} local epochs")
            train_model(client_models[client_idx], trainloader, criterion, optim.Adam(client_models[client_idx].parameters(), lr=0.001), device, local_epochs)

        global_state_dict = global_model.state_dict()
        for key in global_state_dict.keys():
            global_state_dict[key] = sum(client_models[i].state_dict()[key] for i in selected_clients) / len(selected_clients)

        global_model.load_state_dict(global_state_dict)

        for client_idx in selected_clients:
            print(f"Transferring knowledge from client {client_idx}")
            optimizer = optim.Adam(global_model.parameters(), lr=0.001)
            #optimizer = optim.SGD(client_models[client_idx].parameters(), lr=0.001, momentum=0.9)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5) 
            #optimizer = ASFOptimizer(global_model.parameters(), base_lr=0.001)

            train_model_with_ktl(global_model, client_models, trainloader, criterion, optimizer, device, local_epochs)
            scheduler.step() 
...
def main():
    ...
    dataset_path_mapping = {
        'cifar10': ('../data/CIFAR-10/npy/train_images.npy', '../data/CIFAR-10/npy/train_labels.npy', '../data/CIFAR-10/npy/test_images.npy', '../data/CIFAR-10/npy/test_labels.npy'),
        'cifar100': ('../data/CIFAR-100/npy/train_images.npy', '../data/CIFAR-100/npy/train_labels.npy', '../data/CIFAR-100/npy/test_images.npy', '../data/CIFAR-100/npy/test_labels.npy'),
        'fashion_mnist': ('../data/FashionMNIST/npy/train_images.npy', '../data/FashionMNIST/npy/train_labels.npy', '../data/FashionMNIST/npy/test_images.npy', '../data/FashionMNIST/npy/test_labels.npy'),
        'mnist': ('../data/MNIST/npy/train_images.npy', '../data/MNIST/npy/train_labels.npy', '../data/MNIST/npy/test_images.npy', '../data/MNIST/npy/test_labels.npy')
    }
    ...
    if args.loss_function == 'cross_entropy':
        if args.dataset == 'cifar100':
            class_weights = torch.FloatTensor([1.0] * 100).to(device)  
        else:
            class_weights = torch.FloatTensor([1.0, 0.8, 0.5, 0.7, 0.6, 1.0, 0.9, 0.4, 0.6, 1.0]).to(device)
    ...
    if args.model_name in ['SNN_VGG9_BNTT', 'SNN_VGG11_BNTT', 'SNN_VGG9_BNTT_ASF', 'SNN_VGG11_BNTT_ASF',
    ...
    federated_learning_with_gmbs_ktl(model_mapping[args.model_name], trainloader, testloader, criterion, device, args.rounds, args.local_epochs, args.client_selection_ratio, args.clients, model_args, args.model_save_path, args.save_frequency, 5)
